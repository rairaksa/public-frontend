function MenuList() {
    ...
    return (
        data.map(value => (
            <a href="javascript:;" className="w-full h-auto">
                <div className="w-full bg-gradient-to-r from-dark-80 to-dark-60 h-auto text-sm py-4 px-16 border-r-2 border-solid border-avana-orange">
                    { capitalizeFirstWord(value.id) }
                </div>
            </a>
        ))
    )
